...
@group(0) @binding(0) var luma_texture: texture_2d<f32>; // luminance data (from pass 1, 1ch x1)
@group(0) @binding(1) var luma_sampler: sampler; // sampler for luma texture
@group(0) @binding(2) var output_texture: texture_storage_2d<rg32float, write>; // X-direction gradient (2ch x2)
...
fn process(pos: vec2i) {
    ...
    let pt = vec2f(1.0) / vec2f(luma_dims);

    // Sample luma values
    let l = textureSampleLevel(luma_texture, luma_sampler, uv_pos + vec2f(-pt.x, 0.0), 0.0).r;
    let c = textureSampleLevel(luma_texture, luma_sampler, uv_pos, 0.0).r;
    let r = textureSampleLevel(luma_texture, luma_sampler, uv_pos + vec2f(pt.x, 0.0), 0.0).r;
