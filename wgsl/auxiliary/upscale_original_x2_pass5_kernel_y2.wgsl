...
@group(0) @binding(1) var lumamm_texture: texture_2d<f32>; // directional gradient (LUMAMM from pass 4, 2ch x2)
@group(0) @binding(2) var output_texture: texture_storage_2d<rg32float, write>; // normalized directions (LUMAMM, 2ch x2)
...
fn process(pos: vec2i) {
    ...
    let dims = vec2i(textureDimensions(lumamm_texture));
    let tx = textureLoad(lumamm_texture, max(pos + vec2i(0, -1), vec2i(0)), 0).r;
    let cx = textureLoad(lumamm_texture, pos, 0).r;
    let bx = textureLoad(lumamm_texture, min(pos + vec2i(0, 1), dims - 1), 0).r;

    let ty = textureLoad(lumamm_texture, max(pos + vec2i(0, -1), vec2i(0)), 0).g;
    let by = textureLoad(lumamm_texture, min(pos + vec2i(0, 1), dims - 1), 0).g;
