...
// Anime4K-v3.2-Upscale-DTD-x2-Kernel-Y (SIGMA=1.0)

@group(0) @binding(0) var input_texture: texture_2d<f32>; // source texture (HOOKED, 4ch x1)
@group(0) @binding(1) var lineluma_texture: texture_2d<f32>; // luminance texture (LINELUMA, 1ch x1)
@group(0) @binding(2) var mmkernel_texture: texture_2d<f32>; // x-blurred texture (MMKERNEL, 1ch x1)
@group(0) @binding(3) var output_texture: texture_storage_2d<r32float, write>; // output (MMKERNEL, 1ch x1)
...
fn process(pos: vec2i) {
    ...
    let luma_orig = textureLoad(lineluma_texture, pos, 0).x;
    let luma_blur = lumGaussian(pos, d);
    let result = min(luma_orig - luma_blur, 0.0);
