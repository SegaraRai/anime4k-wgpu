...
// Anime4K-v3.2-Upscale-DTD-x2-Kernel-X (Gradients)
...
@group(0) @binding(2) var output_texture: texture_storage_2d<rg32float, write>; // gradient output (LUMAD, 2ch x1)
...
fn process(pos: vec2i) {
    ...
    let l = textureLoad(lineluma_texture, l_pos, 0).x;
    let c = textureLoad(lineluma_texture, c_pos, 0).x;
    let r = textureLoad(lineluma_texture, r_pos, 0).x;
