...
// Anime4K-v3.2-Upscale-DTD-x2-Kernel-X (Gradient Gaussian)
...
@group(0) @binding(2) var output_texture: texture_storage_2d<r32float, write>; // gaussian output (LUMADG, 1ch x1)
...
fn lumGaussian(pos: vec2i, d: vec2i) -> f32 {
    ...
    var g = textureLoad(lumad_texture, pos, 0).x * gaussian(0.0, sigma, 0.0);

    let pos_minus = clamp(pos - d, vec2i(0), bound);
    let pos_plus = clamp(pos + d, vec2i(0), bound);
    g = g + (textureLoad(lumad_texture, pos_minus, 0).x + textureLoad(lumad_texture, pos_plus, 0).x) * gaussian(1.0, sigma, 0.0);
    ...
    for (var i: i32 = 2; i < i32(ceil(kernel_size)); i++) {
        ...
        g = g + (textureLoad(lumad_texture, pos_minus_i, 0).x + textureLoad(lumad_texture, pos_plus_i, 0).x) * gaussian(f32(i), sigma, 0.0);
    }
