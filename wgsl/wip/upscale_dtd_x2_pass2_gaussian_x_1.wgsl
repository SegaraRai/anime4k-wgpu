...
// Anime4K-v3.2-Upscale-DTD-x2-Kernel-X (SIGMA=1.0)
...
fn lumGaussian(pos: vec2i, d: vec2i) -> f32 {
    let dims = textureDimensions(lineluma_texture);
    let bound = vec2i(dims) - 1;
    let s = SIGMA * f32(dims.y) / 1080.0;
    let kernel_size = s * 2.0 + 1.0;

    var g = textureLoad(lineluma_texture, pos, 0).x * gaussian(0.0, s, 0.0);
    var gn = gaussian(0.0, s, 0.0);

    let pos_minus = clamp(pos - d, vec2i(0), bound);
    let pos_plus = clamp(pos + d, vec2i(0), bound);
    g += (textureLoad(lineluma_texture, pos_minus, 0).x + textureLoad(lineluma_texture, pos_plus, 0).x) * gaussian(1.0, s, 0.0);
    ...
    for (var i: i32 = 2; i < i32(ceil(kernel_size)); i++) {
        ...
        g += (textureLoad(lineluma_texture, pos_minus_i, 0).x + textureLoad(lineluma_texture, pos_plus_i, 0).x) * gaussian(f32(i), s, 0.0);
